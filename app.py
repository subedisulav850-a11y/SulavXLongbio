...
def perform_major_login(access_token, open_id):
    ...
    for platform_type in platforms:
        try:
            ...
            edata = binascii.unhexlify(binascii.hexlify(encrypted))
            ...
            if response.status_code == 200:
                try:
                    ...
                    if token:
                        ...
                        last_error = f"Server returned 200 but token is empty (platform {platform_type})"
                except Exception as parse_err:
                    last_error = f"Failed to parse server response (platform {platform_type}): {parse_err}"
            else:
                last_error = f"Server returned HTTP {response.status_code} (platform {platform_type})"

        except requests.exceptions.Timeout:
            last_error = f"Request timed out (platform {platform_type})"
            continue
        except requests.exceptions.ConnectionError as ce:
            last_error = f"Connection error (platform {platform_type}): {ce}"
            continue
        except Exception as e:
            last_error = f"Unexpected error (platform {platform_type}): {e}"
            continue
...
def _resolve_credentials(jwt_token, uid, password, access_token):
    """Shared logic: validate credentials and return (jwt, uid, name, region, open_id, access_token, login_method, error_response)"""
    final_jwt = None
    final_uid = None
    final_name = None
    final_region = None
    ...
    if jwt_token:
        ...
        if acc_token and login_openid:
            ...
            final_jwt, jwt_err = perform_major_login(final_access_token, final_open_id)
            if final_jwt:
                ...
                return None, None, None, None, None, None, None, {
                    ...
                    "code": 500,
                    "reason": jwt_err or "MajorLogin server did not return a token",
                    "hint": "Account may be banned, or Garena server rejected the login"
                }
        else:
            return None, None, None, None, None, None, None, {
                "status": "❌ Invalid Credentials",
                "code": 401,
                "reason": "UID or Password is incorrect — Garena OAuth rejected the request"
            }
        ...
        if not final_uid:
            return None, None, None, None, None, None, None, {
                ...
                "code": 400,
                "reason": "Could not fetch UID — token may be expired or invalid"
            }
        final_open_id = get_openid_from_shop2game(final_uid)
        if final_open_id:
            final_jwt, jwt_err = perform_major_login(access_token, final_open_id)
            if not final_jwt:
                return None, None, None, None, None, None, None, {
                    "status": "❌ JWT Generation Failed",
                    "code": 500,
                    "reason": jwt_err or "MajorLogin server did not return a token",
                    "hint": "Token may be expired or account is banned"
                }
        else:
            return None, None, None, None, None, None, None, {
                "status": "❌ OpenID Fetch Failed",
                "code": 400,
                "reason": "Could not get OpenID — UID may be invalid"
            }
    else:
        return None, None, None, None, None, None, None, {
            "status": "❌ Error",
            "code": 400,
            "reason": "Provide JWT, UID/Password, or Access Token"
        }

    return final_jwt, final_uid, final_name, final_region, final_open_id, final_access_token, login_method, None
...
def authorise():
    ...
    access_token = request.args.get("access_token") or request.args.get("access") or request.form.get("access_token") or request.form.get("access")

    final_jwt, final_uid, final_name, final_region, final_open_id, final_access_token, login_method, err = \
        _resolve_credentials(jwt_token, uid, password, access_token)

    if err:
        return jsonify({**CREDITS, **err}), err["code"]
...
def combined_bio_upload():
    ...
    if not bio:
        return jsonify({**CREDITS, "status": "❌ Error", "code": 400, "reason": "Missing 'bio' parameter"}), 400
